...
private:
	...
	void gc() {
		...
		while (iter != m_all_conn.end()) {
			if (iter->second->idle == 0)continue;
			double diff =  difftime( iter->second->idle, now);
			std::cout << "diff=" << diff << std::endl;
			if (diff> m_iocp_time_out) {
				iter->second->close();
			}
			iter++;
		}
...
int main()
{
	...
	while (true)
	{
		long idle = (long)time(0);

		Sleep(3000);
		long now = (long)time(0);
		std::cout << "diff" << now -idle<< std::endl;
