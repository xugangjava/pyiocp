...
void py_call(int conn_id, int event_type, int msg_type, std::string* buf) {
	call<void>(on_py_event, conn_id, event_type, msg_type, *buf);
	delete buf;
}
	...
	public:
		...
		void handle_read_head(const boost::system::error_code& error, size_t bytes_transferred) {
			if (error)
			{
				...
				int msg_type = (m_head[3] & 0x000000ff) << 8 | (m_head[2] & 0x000000ff);
				int sz = total_len - 4;
				boost::asio::async_read(
					m_socket,
					boost::asio::buffer(m_body, sz),
					m_strand.wrap(boost::bind(
		...
		void handle_read_body(int msg_type,int sz, const boost::system::error_code& error, size_t bytes_transferred) {
			if (error)
			{
				close();
			}
			else {
				m_server->send_new_py_event(_id, EVT_ON_MESSAGE, msg_type, new std::string(m_body, sz));
				recv();
			}
		}
		...
		void close() {
			...
			m_server->send_new_py_event(_id, EVT_DISCONNECT,0,new std::string(""));
...
public:
	...
	void send_new_py_event(int conn_id, int event_type, int msg_type, std::string* buf) {
		//python消息队列
	...
	void handle_accept(conn_ptr new_connection, const boost::system::error_code& error) {
		if (!error)
		{
			...
			send_new_py_event(new_connection->get_id(), EVT_CONNECT,0, new std::string(sIp));
...
public:
	...
	void call_on_time(boost::asio::deadline_timer* t, std::string* timer_id, const boost::system::error_code& error) {
		delete t;
		if (!error) {
			m_server->send_new_py_event(0, EVT_ON_TIMER,0,timer_id);
		}
	}
