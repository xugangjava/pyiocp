...
struct py_event {
	...
	std::string get_conn_id() {
		return std::string(conn_id, c_sz);
	}
};
...
class server
{
	...
	//		packet.conn_id = id;
			packet.c_sz = id.size();
			strncpy_s(packet.conn_id, uuid_length, id.c_str(), packet.c_sz);
		...
		void handle_read_body(const boost::system::error_code& error, size_t bytes_transferred) {
			if (error)
			{
				...
				strncpy_s(packet.buf, packet_length, body, packet.sz);
		...
		void check() {
			if (!is_open)return;
			//防止空连接
			if (difftime(time(0), active_time) > m_server->m_iocp_time_out) {
...
//	boost::asio::io_service::strand m_strand;
	...
	boost::lockfree::queue<py_event> queue;
...
private:
	...
	void handle_accept(conn_ptr conn, const boost::system::error_code& error) {
		...
		//	packet.conn_id = conn->id;
			...
			packet.sz = sIp.size();
			//packet.buf = sIp;
			strncpy_s(packet.buf, packet_length, sIp.c_str(), packet.sz);
			conn->is_open = true;
...
public:
	...
	void run(PyObject* cb, int port,int iocp_time_out) {
		...
		//while (1) {
		//	if (m_server->queue.empty())continue;
		//	py_event evt;
		//	m_server->queue.pop(evt);
		//	call<void>(on_py_event,evt);
		//}
	}
	...
	void call_on_time(boost::asio::deadline_timer* t, std::string& timer_id, const boost::system::error_code& error) {
		...
		if (!error) {
			...
			//evt.buf = timer_id;
			evt.sz = timer_id.size();
			strncpy_s(evt.buf, packet_length, timer_id.c_str(), evt.sz);
